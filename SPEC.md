...
- 2026-06-02: Fixed three related bugs causing total write lockout for @autonomous in downstream projects: (1) SDK path key wrong — fallback used `path: { sessionID }` but SDK requires `path: { id }`, so fallback always threw and was swallowed, leaving identity unresolved. (2) Fail-closed policy too aggressive — when identity was unresolved AND any `write_allowlist` existed, ALL writes were blocked including files not covered by any agent rule. Fixed to C1 policy: unknown identity only blocks files explicitly named in `prometheus_only` or some agent's `write_allowlist`; uncovered files are allowed. (3) Child/subagent session identity not inherited — added `parentID` chain walk via `client.session.get({ path: { id } })` so task-delegated sessions inherit the originating agent identity. Added `tests/plugins/immutability.test.mjs` (18 unit tests, 100% pass) covering all three fixes. All 42 integration checks pass.
