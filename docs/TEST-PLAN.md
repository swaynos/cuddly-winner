...
| Test case | Use case | Class | Setup and action | Evidence | Pass condition |
| --- | --- | --- | --- | --- | --- |
| TP-ID-01 | UC-ID-01 | U | Exercise direct managed sessions, descendants, managed children below unmanaged parents, switching into and out of pinning and read-only identities, conflicting identities, and cycles. | Resolved identity and each tool decision. | The topmost pinning ancestor, or a currently read-only ancestor, determines the boundary. A Prometheus or generated identity pins for the session lifetime; an Ask, Grounder, or Reviewer selection binds only its own turn, so returning to Plan or Build restores write and Bash. Delegation cannot widen access, and ancestry cycles fail closed for managed mutation and Bash. |
