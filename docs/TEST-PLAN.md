...
| TP-RESOURCE-03 | UC-RESOURCE-03 | U, F | Exercise ephemeral contexts, headed login capture, pre-login state baselines, combined completion predicates, cookies, local storage, explicit session storage, origin checks, expiry, status, and removal. | State metadata, completion decisions, file modes, browser contexts, redacted logs, and cleanup result. | Login cannot complete on a redirect without approved-origin state change; every supplied predicate must match; state values never enter output; unsafe paths and origins fail; saved state works headlessly; removal affects only the selected record. |
