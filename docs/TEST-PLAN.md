...
| TP-AUT-12 | UC-AUT-12 | S, B | Give Autonomous an exhausted safe-path blocker. | One creative-pass attempt at a safe, reversible alternative within unchanged scope, the block report, and any feedback record. | It takes one creative pass at a safe, reversible alternative within the unchanged requested outcome, acceptance criteria, and permissions; on a confirmed block it reports the failed step, a concise blocker code, and the exact next human action, recording via `cuddly-winner-feedback` when available. It does not delegate to a recovery agent, emit a strict terminal record, loop, or widen scope. |
...
| TP-DEP-04 | UC-DEP-04 | F | Prepare current copies, repository symlinks, modified copies, foreign symlinks, unrelated entries, and an arbitrary retired agent recorded in prior managed state. Run status, install, and removal without profile flags. | Status classifications, managed state, and final filesystem state. | Every current managed group is inspected. Install removes only a recorded retired agent whose copy or repository link remains unchanged; modified and unrelated entries remain. |
