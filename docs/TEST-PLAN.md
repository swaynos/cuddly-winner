...
| TP-AUT-05 | UC-AUT-05, UC-AUT-06 | S, B | Exercise successful completion, repeated lack of progress, exhausted declared limits, and a concrete blocker. Inspect remaining edits, Git history, Git index, parent handoff, detailed PR Contract, and validator task result. | Stop condition, concise parent report, worktree, index, detailed evidence packet, and validator report. | Autonomous stops at the applicable documented condition, leaves work visible, never mutates Git publication state, reports fresh verification and a `VALIDATED` verdict before successful status, and keeps detailed validator evidence in its delegated task result. |
...
| TP-RESOURCE-02 | UC-RESOURCE-02 | U, S | Configure an unauthenticated NotebookLM profile (no `notebooklm login` run yet) and inspect the managed MCP entry and its tool surface. | MCP configuration, exposed tool list, and agent response. | The managed entry resolves to the shared project virtualenv; `server_info` reports no usable session; no auth or repair tool is exposed for the agent to call, authenticated or not. |
