...
| TP-ID-04 | UC-ID-04, UC-VAL-01 | U | For Ask, Karpathy, Reviewer, Grounder, Implementation Validator, and Out-of-the-box Thinker, request every mutation tool, command execution, and delegated implementation. | Tool decisions, child-session activity, and filesystem state. | All mutation and command execution is denied, including attempts to widen the role through delegation. |
