...
| Test case | Use case | Class | Setup and action | Evidence | Pass condition |
| --- | --- | --- | --- | --- | --- |
| TP-ID-01 | UC-ID-01 | U | Exercise direct managed sessions, descendants, managed children below unmanaged parents, explicit root-agent switches, and cycles. | Resolved identity and each tool decision. | A root uses its current agent, so switching from Prometheus or a generated agent to Plan or Build restores native access. A managed parent still constrains a child for its current turn. Delegation cannot widen access, parent-ancestry cycles fail closed, and root authorization does not require message history. |
...
| Test case | Use case | Class | Setup and action | Evidence | Pass condition |
| --- | --- | --- | --- | --- | --- |
| TP-RESOURCE-01 | UC-RESOURCE-01 | S, B | **B BLOCKED until Phase 5.** Use future fixture `tests/fixtures/agent_value/resource-selection.md` with direct URLs and rendered-only content. | Tool order, browser mode, rationale, and approval event. | Direct retrieval precedes browser use. Rendered work uses the configured headless or virtual-display task mode. A separate visible window serves approved human login. |
| TP-RESOURCE-02 | UC-RESOURCE-02 | S, U, F | Inspect the deployed rule and durable contract; exercise both task modes, visible and hidden controls, rich text, disabled state, bounded waits, recovery, access denial, login detection, approved visible login, return to task work, missing GUI, launch failure, exit, timeout, and interrupted actions. | Modes, actionability, fill evidence, waits, page health, approval, state transitions, processes, and action records. | Playwright is the only backend. Hidden fallbacks are omitted, rich controls work, disabled actions fail before dispatch, fills are read back, waits preserve the context, navigation classifies denial, and unknown actions are inspected rather than replayed. |
| TP-RESOURCE-03 | UC-RESOURCE-03 | U, F | Exercise detached start, launcher exit, explicit completion, duplicate start, cancellation, window closure, approved popups, local and session storage, origin checks, status, and removal. | Process lifetime, private socket and state modes, command responses, headless verification status, redacted output, and cleanup result. | Start returns without capture or login polling. Only complete saves user-confirmed state. Cancel and window closure preserve prior records. Fresh headless restoration permits inspection without claiming authentication. Unsafe paths and origins fail; secrets never enter output; removal affects only the selected record. |
| TP-RESOURCE-04 | UC-RESOURCE-04 | U, F | Install, repeat, diagnose, modify one managed entry, and remove against a configuration containing user entries and retired project-owned browser entries. | Backups, exact JSON, status, and final configuration. | User entries survive; the Playwright entry is idempotent; proved retired entries are removed; modified entries remain; diagnostics do not launch a browser. |
| TP-RESOURCE-05 | UC-RESOURCE-05 | U, F | Use headed and headless Playwright fixtures with a configured site. Exercise approval, completion, request, redirects, expiry, close, capacity, and deployment. | Opaque handle, response metadata, request headers, state bytes, modes, and installed tool. | Login alone uses headed mode; credentials never enter output; only bounded read-only same-origin requests run headlessly; close and expiry remove session state. |
| TP-RESOURCE-06 | UC-RESOURCE-06 | U, F | Trigger a browser download, authenticated visible media without a download control, a stale source fingerprint, a path collision, and an interrupted transfer. | Page association, Playwright download event or internal media retrieval, dimensions, output bytes, source and output hashes, path, and action state. | Success requires a new result tied to the request, no source-URL disclosure, an exclusively placed local file, and valid bytes. A stale item or unknown transfer cannot pass. |
| TP-RESOURCE-10 | UC-RESOURCE-02 | U, F | Run `browser_virtual_display.test.mjs` and runtime/configuration/deployment fixtures with Xvfb and xauth installed; CI installs these host packages. | Actual headed launch, 1280x1024 viewport and screen, distinct concurrent displays, restored scoped state, JSON-RPC, exit status, EOF and signal cleanup, missing-dependency errors, and mode reporting. | The actual MCP owns its virtual display; human login keeps its physical display; no automatic fallback occurs. Skipped real-Xvfb coverage is not acceptance. |
| TP-RESOURCE-11 | UC-RESOURCE-07, UC-RESOURCE-06 | U, F | Run `browser_image_transfer.test.mjs` in headless and virtual-display modes, plus `browser_downloads.test.mjs`. Repeat the transfer fixture against installed Chrome. | Exact uploaded/downloaded PNG bytes, MIME/signature/hash, chooser and direct-input selection, refs, authenticated retrieval, media saving, invalid sources, disabled/hidden controls, collisions, and timeout recovery. | No rejected source is transmitted. Upload does not submit a prompt. Existing output files survive. Event listeners clean up, and image delivery requires local byte checks. |

`browser_runtime.test.mjs` covers shared settings precedence, mode/channel
selection, compatibility options, and origin-scoped storage restoration.
`browser_login_handoff.test.mjs` uses a headless Chromium fixture to check worker
lifetime across launcher exit and separate completion, cancellation, and closure.
`browser_login_capture.test.mjs` covers approved popups without synthetic tabs.
The MCP suite covers missing/foreign state, pending authentication after capture,
and blocked credential-reading evaluation. These fixtures do not prove live site
acceptance or a physical desktop login.
