...
| Test case | Use case | Class | Setup and action | Evidence | Pass condition |
| --- | --- | --- | --- | --- | --- |
| TP-RESOURCE-01 | UC-RESOURCE-01 | S, B | **B BLOCKED until Phase 5.** Use future fixture `tests/fixtures/agent_value/resource-selection.md` with direct URLs and rendered-only content. | Tool order, browser mode, rationale, and approval event. | Direct retrieval precedes browser use. Rendered work uses headless Playwright. Headed Playwright is used only for an approved human login. |
| TP-RESOURCE-02 | UC-RESOURCE-02 | S, U, F | Inspect the deployed rule and durable contract; exercise headless startup, visible and hidden controls, rich text, disabled state, bounded waits, closed-page recovery, access denial, login detection, approved headed login, return to headless work, missing GUI state, launch failure, browser exit, timeout, and interrupted actions. | Ordered rule clauses, modes, actionability, fill evidence, wait duration, page health, approval, state transitions, process state, and action records. | Playwright is the only backend. Hidden fallbacks are omitted, rich controls work, disabled actions fail before dispatch, fills are read back, waits preserve the context, navigation recovers a page and classifies denial, and unknown actions are inspected rather than replayed. |
| TP-RESOURCE-03 | UC-RESOURCE-03 | U, F | Exercise ephemeral contexts, headed login capture, mandatory account selectors, pre-login state baselines, combined completion predicates, cookies, local storage, explicit session storage, origin checks, expiry, status, and removal. | State metadata, selector visibility, completion decisions, file modes, browser contexts, redacted logs, and cleanup result. | Login cannot complete without approved-origin state change and visible account evidence; every supplied URL or cookie predicate must match; the fresh headless context must see the saved selector; selector-less or obsolete state requires recapture; state values never enter output; unsafe paths and origins fail; removal affects only the selected record. |
