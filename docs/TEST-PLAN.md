...
| TP-RESOURCE-02 | UC-RESOURCE-02 | S, U, F | Inspect the deployed rule, image skill, and durable skill contract; exercise cross-platform browser resolution, invalid executables, missing Linux GUI state, spawn errors, early browser exit, stalled DevTools traffic, unsafe session paths, forced cleanup, and managed installation. | Ordered rule clauses, skill gates, resolver results, failure time and message, child-process state, temporary-profile state, protected target bytes, and installed control files. | Project overrides take precedence; otherwise the agent starts in Obscura, checks whether login is required, and uses the user's browser only for needed login. Missing GUI, failed starts, and stalled DevTools operations return within the capture deadline. Unsafe destinations fail without escaping the configuration root, capture stops the browser before profile removal, and the helper installs as a managed copy whose modified form survives removal. |
