...
| TP-RESOURCE-02 | UC-RESOURCE-02 | S, U, F | Inspect the deployed rule and durable contract; exercise headless startup, login detection, approved headed login, return to headless work, missing GUI state, launch failure, browser exit, timeout, and interrupted actions. | Ordered rule clauses, modes, approval, state transitions, process state, and action records. | Playwright is the only backend. Headed mode starts only after login is proved necessary and approval is granted. It closes before headless task work resumes. Unknown actions are inspected, never replayed automatically. |
| TP-RESOURCE-03 | UC-RESOURCE-03 | U, F | Exercise ephemeral contexts, headed login capture, cookies, local storage, explicit session storage, origin checks, expiry, status, and removal. | State metadata, file modes, browser contexts, redacted logs, and cleanup result. | State values never enter output; unsafe paths and origins fail; saved state works headlessly; removal affects only the selected record. |
...
| TP-RESOURCE-06 | UC-RESOURCE-06 | U, F | Trigger a browser download, a generated preview without a download, a stale page item, a path collision, and an interrupted transfer. | Page association, Playwright download event, output bytes, hash, path, and action state. | Success requires a new result tied to the request, an exclusively placed local file, and valid bytes. A preview, stale item, or unknown transfer cannot pass. |
