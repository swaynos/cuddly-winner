...
| Group | Requirement |
| --- | --- |
| Installation | The installer deploys four agents (`ask`, `grounder`, `prometheus`, `reviewer`), all three plugins, all four workflow tools, the pinned SDK, the pinned Playwright library, the headless Playwright browser MCP server, its secure login-state store, and the headed login helper. |
