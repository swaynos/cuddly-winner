...
| Group | Requirement |
| --- | --- |
| Installation | The installer deploys four agents (`ask`, `grounder`, `prometheus`, `reviewer`), all three plugins, all four workflow tools, the pinned SDK, the pinned Playwright library, the pinned Obscura browser engine binary, and the credential-substitution wrapper it launches. |
